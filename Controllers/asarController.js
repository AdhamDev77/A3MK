...
const createAsar = async (req, res) => {
    ...
    try {
        ...
        if (!project_info || !project_goals_1 || !project_goals_2 || !project_tahlils || !m3neen || !project_natiga || !mod5alatNumber || !kema_mogtama3ya || !safy_kema_mogtama3ya || !aed) {
            return res.status(400).json({ error: "يجب ملأ جميع الخانات من فضلك" });
        }

        // Create comm
        const asar = await Asar.create({
            project_info, project_goals_1 , project_goals_2, project_tahlils, m3neen, project_natiga, mod5alat: mod5alatNumber, kema_mogtama3ya, safy_kema_mogtama3ya, aed
        });

        const comm = await Comm.findById(id)
        await Asar.findByIdAndDelete(comm.comm_asar);
        comm.comm_asar = asar._id;
        await comm.save();
